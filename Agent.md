...
workflow_compare_ui.py
  獨立的多工作流比較與合成視窗。讀取 export_pair 與 inpaint_workflows，按各工作流結果和底圖的差異生成專屬 Mask，以同步比較滑桿、矩形或筆刷指定來源，最左側顯示當頁成品並輸出 result；不依賴 other_mask。
...
魔法棒工具
多邊形套索工具
局部 ROI 獨立編輯視窗，含獨立的四邊調整工具，所有編輯工具硬限制於 ROI 內
撤銷 / 重做
自動保存 mask
自動重新生成當前頁 overlay
生成 PDF 預覽
