...
左側預覽沿用舊版「自動」的顯示：偵測文字 Mask（限制在純色填充類內）預設白色，背景取樣區以固定 28% 不透明度疊加黃色；實際整區填色 overlay 只供右側成品預覽使用。人工新增範圍也會顯示，擦除後不再顯示白色。滑桿 100% 為黑底文字 Mask，0% 為原圖；黃色背景取樣提示由「顯示背景選區」獨立控制。預設 70%，沿用舊版保存的滑桿比例。筆刷及局部視窗使用相同顯示規則。
