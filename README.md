...
自動分類依氣泡輪廓區分取樣方式：氣泡內保留局部背景取樣與方向回退；氣泡外同時檢查緊鄰文字的 3 px 環形取樣與向外延伸 12 px 的背景，上下左右四個方向均需有足夠樣本並通過純色檢查，避免只取到文字白色描邊。未偵測到氣泡或停用氣泡功能時，使用氣泡外的保守判斷。氣泡檢查只擴展已確認的純色填充；擴展失敗保留局部填充，圖像修補區域會阻止擴展覆蓋。既有頁面需執行「重新分類」才套用新規則，人工修改仍會保留。

「使用傳入 Mask 運行」的取代模式以傳入 Mask 重新分類並保留人工決定；交集模式同時收窄兩類並保留填色顏色。沒有同名 PNG 的頁面保持原狀。

「從其他轉入」與批次轉成「圖像修補」時，會撤掉選中的純色填充，只將文字修補範圍轉入待修復 Mask，不包含自動擴展的氣泡背景；人工添加範圍與既有圖像修補範圍保留。缺少有效文字偵測快取時保留原範圍，避免誤刪人工選區。
